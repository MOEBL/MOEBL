 ...
 *
 * DESCRIPTION:
 * Render OPDS library/catalog class file.
 */
        ...
        $xml->startDocument('1.0','UTF-8');
            $xml->startElement ("OpenSearchDescription");
                ...
                $xml->startElement ("Url");
                    ...
                    $xml->writeAttribute ("template", $config['moebl_full_url'] . 'feed.php?query={searchTerms}');
                $xml->endElement ();
        ...
        self::getXmlStream ()->startElement ("feed");
            ...
            self::getXmlStream ()->startElement ("author");
                self::getXmlStream ()->startElement ("name");
                    self::getXmlStream ()->text (utf8_encode ("My Online eBook Library"));
                self::getXmlStream ()->endElement ();
                self::getXmlStream ()->startElement ("uri");
                    self::getXmlStream ()->text ("http://moebls.x10.mx");
                self::getXmlStream ()->endElement ();
                self::getXmlStream ()->startElement ("email");
                    self::getXmlStream ()->text ("moeblib@gmail.com");
                self::getXmlStream ()->endElement ();
            self::getXmlStream ()->endElement ();
            ...
            self::renderLink ($link);
            if ($config['moebl_opds_generate_invalid_stream'] == 0 || preg_match("/(MantanoReader)/", $_SERVER['HTTP_USER_AGENT'])) {
                // Good and compliant way of handling search
                $link = new Link ("feed.php?page=" . self::PAGE_OPENSEARCH, "application/opensearchdescription+xml", "search", "Search here");
            }
            else
            {
                // Bad way, will be removed when OPDS clients are fixed
                $link = new Link ($config['moebl_full_url'] . 'feed.php?query={searchTerms}', "application/atom+xml", "search", "Search here");
            }
            self::renderLink ($link);
    }
